...
function computeRescueTarget(
  ...
  if (feeType === "percent" && includeFees) {
    ...
    const f = includeFees ? (feeType === "flat" ? feeValue : 0) : 0;
...
function InsightsTab({ holding, marketPrice, cp, onUseInCalculator, onSaved }: {
  ...
  const rescueTargets = useMemo(() => {
    ...
    const levels = [0.15, 0.35, 0.55]; // modest, moderate, ambitious
    return levels
      .map((pct) => {
  ...
  const getEfficiencyBand = (score: number) => {
    ...
    if (score >= 60) return { label: "Good opportunity", color: "text-primary" };
    if (score >= 40) return { label: "Neutral", color: "text-muted-foreground" };
    if (score >= 20) return { label: "Weak", color: "text-muted-foreground" };
    return { label: "Inefficient", color: "text-destructive" };
  };

  // ── What If I Started Today ─────────────────────────────────
  const startTodayDiff = useMemo(() => {
    if (marketPrice == null || A <= 0) return null;
    const diff = ((A - marketPrice) / A) * 100;
    return diff; // positive = new buyer is better, negative = your position is better
  }, [A, marketPrice]);
  ...
  const handleSaveRescue = (target: { target: number; budget: number; shares: number; newAvg: number }) => {
    ...
    addScenario({
      ...
      include_fees: true, fee_amount: 0,
      buy_price: marketPrice, shares_to_buy: target.shares,
      budget_invested: target.budget, fee_applied: 0,
      total_spend: target.budget, new_total_shares: S + target.shares,
      new_avg_cost: target.newAvg, recommended_target: null,
    ...
    <div className="space-y-5">
      ...
      <div className="rounded-xl border border-border bg-card p-4 sm:p-5">
        ...
        <p className="text-[11px] text-muted-foreground mb-4">
          Capital required to lower your average cost to meaningful levels at {cp}{fmt2(marketPrice)}/share.
        </p>

        {rescueTargets.length === 0 ? (
          <p className="text-xs text-muted-foreground/70">
            {marketPrice >= A
              ? "Current price is at or above your average — no rescue needed."
              : "Unable to compute rescue targets."}
          </p>
        ) : (
          <div className="space-y-2.5">
            {rescueTargets.map((t, i) => {
              const labels = ["Modest", "Moderate", "Ambitious"];
              return (
      ...
      {efficiencyScore != null && (
        <div className="rounded-xl border border-border bg-card p-4 sm:p-5">
          ...
          <div className="flex items-baseline gap-3 mb-2">
            <span className={`text-4xl font-mono font-bold ${getEfficiencyBand(efficiencyScore).color}`}>
              {efficiencyScore}
            </span>
          ...
          <div className="mt-3 pt-3 border-t border-border">
            ...
            <div className="flex justify-between mt-1">
              ...
              <span className={`text-[9px] font-medium ${getEfficiencyBand(efficiencyScore).color}`}>
                {getEfficiencyBand(efficiencyScore).label}
              </span>
